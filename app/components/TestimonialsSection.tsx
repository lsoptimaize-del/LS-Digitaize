      ...
      <motion.div style={{ display: 'flex', flexDirection: 'column', gap: 10, y: yPercent, willChange: 'transform' }}>
        {doubled.map((t, i) => (
          <button
            ...
            style={{
              width: cardWidth,
              background: 'rgba(255,255,255,0.95)', // Solidified background slightly to replace blur
              ...
              textAlign: 'left',
              flexShrink: 0,
              boxShadow: '0 8px 30px rgba(0,0,0,0.04), inset 0 0 0 1px rgba(255,255,255,0.8)',
...
export default function TestimonialsSection() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (w < 480) { setCols(2); setCardWidth(Math.min(186, (w - 44) / 2)); }
    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', zIndex: 0, pointerEvents: 'none' }}>
        <div style={{
          ...
          filter: 'blur(80px)', animation: 'testimDrift 15s ease-in-out infinite alternate'
        }} />
        <div style={{
          position: 'absolute', bottom: '10%', right: '5%', width: '50vw', height: '50vw',
          background: 'radial-gradient(circle, rgba(91, 201, 232, 0.2) 0%, transparent 70%)',
          filter: 'blur(90px)', animation: 'testimDrift 20s ease-in-out infinite alternate-reverse'
        }} />
        <div style={{
          position: 'absolute', top: '35%', left: '40%', width: '40vw', height: '40vw',
          background: 'radial-gradient(circle, rgba(160, 210, 255, 0.18) 0%, transparent 70%)',
          filter: 'blur(70px)', animation: 'testimDrift 18s ease-in-out infinite alternate'
        }} />
      </div>
      <style>{`
        @keyframes testimDrift {
          0% { transform: translate(0, 0) scale(1); }
          100% { transform: translate(4vw, 6vh) scale(1.15); }
        }
      `}</style>
      ...
      <div style={{ flex: 1, overflow: 'hidden', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', perspective: '900px', perspectiveOrigin: '50% 38%' }}>
        ...
        <div style={{ pointerEvents: 'none', position: 'absolute', inset: 0, background: `linear-gradient(to right, #FAFAF7 0%, transparent ${isMobile ? 4 : 12}%, transparent ${isMobile ? 96 : 88}%, #FAFAF7 100%)` }} />
      </div>
      ...
      <div
        ...
        style={{
          ...
          background: [
            ...
            `linear-gradient(to right, #FAFAF7 0%, transparent ${isMobile ? 3 : 11}%, transparent ${isMobile ? 97 : 89}%, #FAFAF7 100%)`,
          ].join(', '),
        }}
      />
